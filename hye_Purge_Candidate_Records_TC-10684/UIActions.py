...
try: 
    from condition import Mathmatic, Assertion
except Exception as e:
    pass
...
def switch_to_frame_by_xpath(driver: webdriver.Chrome, frame_info: dict): 
    try:
        ...
        print(f"Error in switch_to_frame_by_xpath: {e}")
        return None
...
class Heal:
    ...
    def get_outer_html(self) -> requests.Response:
        if self.mobile_tagify:
             self.driver.execute_script(self.mobile_tagify)
        ...
        xpath_mapping = self.execute_async_js("fetchJsonData(\"JSONOutput\")")
        xpath_mapping = json.loads(xpath_mapping)
        ...
        try:
            for key, value in xpath_mapping.items():
                ...
                # 2) Switch to the appropriate frame/shadow context, if any
                try:
                    if frame_info and frame_info != "":
                        self.driver.switch_to.default_content()
                        _ = switch_to_frame_by_xpath(driver=self.driver, frame_info=json.dumps(frame_info))
                except Exception:
                    pass
                try:
    ...
    def vision_query_v2(self) -> requests.Response:
        attempt = 1
        max_attempt = 2
...
def vision_query(driver: webdriver.Chrome, operation_index: str):
    ...
    try:
        ...
        if get_meta_data_value(operation_index).get("version") == "v2":
            response = Heal(operation_index, driver).vision_query_v2()
        else:
...
def textual_query(driver: webdriver, operation_index: str):
    heal = Heal(operation_index, driver)
    outer_html = heal.get_outer_html()
    response = heal.textual_query(outer_html)
    if 'regex' in response:
        pattern = response.get('regex')
        match = re.search(pattern, outer_html)
        if match:
            return match.group(0)
        else:
            raise Exception("No match found for the regex in the outer HTML")
    else:
        raise Exception("Error in Getting Regex")
...
def ui_action(driver: webdriver.Chrome, operation_index: int = 0):
    ...
    non_driver_operation = get_meta_data_value(operation_index).get('non_driver_operation', False)
        ...
        try:
            ...
            if action.lower() == "textual_query":
                ...
                if non_driver_operation:
                    ...
                    math_result_variable_name = Mathmatic.from_json(get_meta_data_value(operation_index)["expression_tree"]).evaluate(user_variables, get_variable_value)
                ...
                if non_driver_operation:
                    print("computing assertion")
                    
                    assertion_value = Assertion.from_json(get_meta_data_value(operation_index)["expression_tree"]).evaluate(user_variables, get_variable_value)
                ...
                flow = GenerativeFlow(
                    ...
                    mobile_tagify_script=MOBILE_TAGIFY_SCRIPT
                )
