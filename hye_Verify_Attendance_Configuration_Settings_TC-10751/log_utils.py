...
# Prevent conflicts with lambdatest logger by using isolated loggers
def create_isolated_logger(name):
    """Create a logger that doesn't propagate to root logger to avoid conflicts"""
...
# Get the main logger - now isolated from root logger
logger = create_isolated_logger(__name__)
...
def log_autoheal_selector(org_id = "", test_id = "", test_version = "", ktm_email = "", username = "", execution = None, authoring= None):
    ...
    logger.debug(autoheal_info.to_dict())

# Create isolated loggers for stdout and stderr
stdout_logger = create_isolated_logger('STDOUT')
stdout_logger.setLevel(logging.INFO)
stdout_logger.addHandler(var_log_handler)
stdout_logger.addHandler(stream_handler)

stderr_logger = create_isolated_logger('STDERR')
...
def get_logger(name):
    """
    Get an isolated logger that won't conflict with the lambdatest logger.
    Use this function instead of logging.getLogger() directly.
    """
    return create_isolated_logger(name)